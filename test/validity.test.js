...
describe('validity', function() {
  describe('#booleanToCallback()', function() {
    it('should return a function', function() {
      assert.strictEqual(typeof validity.booleanToCallback, 'function')
    })

    it('should correctly wrap a boolean function: true', function(done) {
      validity.booleanToCallback(Number.isInteger)('age', { age: 1 }, function(
        error,
        valid
      ) {
    ...
    it('should correctly wrap a boolean function: false ', function(done) {
      validity.booleanToCallback(Number.isInteger)(
        'age',
        { age: 'a' },
  ...
  describe('#createValidator()', function() {
    ...
    it('should return custom failure message when invalid', function(done) {
      ...
      validate('a', 'z', { a: false }, function(error, valid) {
        assert.strictEqual(error, null)
        assert.strictEqual(valid, 'z is not valid')
        done(error)
      })
    })

    it('should return custom failure message injecting #{name} and #{value}', function(done) {
      const validate = validity.createValidator(alwaysFail)(
        'An #{name} with a value of #{value}, is not valid'
      )
      validate('email', 'email', { email: 'paul@serby.net' }, function(
        error,
        valid
      ) {
        assert.strictEqual(error, null)
        assert.strictEqual(
          valid,
          'An email with a value of paul@serby.net, is not valid'
        )
        done(error)
      })
    })
    ...
    it('should return error message an unexpected error occurred', function(done) {
      const validate = validity.createValidator(isValueWithExpectedError)(
        '#{name} is not valid'
      )
      validate('a', 'z', { a: false }, function(error, valid) {
        assert(error instanceof Error)
        assert.strictEqual(valid, 'Unexpected error')
        done()
      })
    })
  })
  ...
  describe('#createValidatorAllowingFailureMessageOverride()', function() {
    it('should return undefined when on successful validated', function(done) {
      const validate = validity.createValidatorAllowingFailureMessageOverride(
        isValue,
        '#{name} is not valid'
      )
      validate('a', 'a', { a: true }, function(error, valid) {
    ...
    it('should all the failure message to be set', function(done) {
      ...
      validate.setFailureMessage('You need a name #{name}')(
        'a',
        'z',
        { a: false },
        function(error, valid) {
          assert.strictEqual(error, null)
          assert.strictEqual(valid, 'You need a name z')
          done(error)
        }
      )
    })
